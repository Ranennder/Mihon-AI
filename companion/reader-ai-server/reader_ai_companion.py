    ...
    updater_script.write_text(
        "\n".join(
            [
                ...
                "      throw 'Installed file size does not match the download'",
                "    }",
                "    $installed = $true",
                "    Add-Content -LiteralPath $updateLog -Value \"Installed on attempt $attempt\"",
                "    break",
                "  } catch {",
                "    Add-Content -LiteralPath $updateLog -Value \"Attempt $attempt failed: $($_.Exception.Message)\"",
                "    Start-Sleep -Milliseconds 500",
                "  }",
                "}",
                "if (-not $installed) {",
                "  Add-Content -LiteralPath $updateLog -Value 'Update failed after 120 attempts'",
                "  exit 1",
                "}",
