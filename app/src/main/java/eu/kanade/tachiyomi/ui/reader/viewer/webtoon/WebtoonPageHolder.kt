...
class WebtoonPageHolder(
    ...
    private val pageUpscaler: ReaderPageUpscaler by injectLazy()
    ...
    private suspend fun setImage() {
        ...
        try {
            ...
            if ((blockingReload || requireUpscaledImage) && pageUpscaler.isEnabled() && !cacheAvailableAfterLoad) {
                showAiFailureToast()
            } else if (!blockingReload && !requireUpscaledImage && pageUpscaler.isEnabled()) {
                if (cacheAvailableAfterLoad || !cacheAvailableBeforeLoad) {
    ...
    private fun showAiFailureToast() {
    ...
    private fun awaitUpscaledImageAndReload(expectedPage: ReaderPage) {
        cacheWatchJob?.cancel()
        cacheWatchJob = scope.launch {
            repeat(1_200) {
                if (!pageUpscaler.isEnabled() || page !== expectedPage) {
                    return@launch
                }
                if (pageUpscaler.hasCachedPage(expectedPage)) {
                    // Avoid re-decoding and swapping a visible webtoon page while the list is
                    // still moving, which causes noticeable hitching during flings.
                    if (viewer.recycler.isInteractionInProgress || !frame.isAttachedToWindow) {
                        delay(500)
                        return@repeat
                    }
