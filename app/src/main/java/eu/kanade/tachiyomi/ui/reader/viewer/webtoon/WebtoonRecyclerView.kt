...
class WebtoonRecyclerView @JvmOverloads constructor(
    ...
    var scrollStateListener: ((Boolean) -> Unit)? = null
    ...
    val isInteractionInProgress: Boolean
        get() = isScrollInProgress || isManuallyScrolling || isZooming || scrollState != SCROLL_STATE_IDLE
    ...
    override fun onScrollStateChanged(state: Int) {
        ...
        val isScrolling = state != SCROLL_STATE_IDLE
        if (isScrollInProgress != isScrolling) {
            isScrollInProgress = isScrolling
            scrollStateListener?.invoke(isScrolling)
        }
