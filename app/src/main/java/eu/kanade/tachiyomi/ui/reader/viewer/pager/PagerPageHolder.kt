...
class PagerPageHolder(
    ...
    private fun initProgressIndicator() {
        if (progressContainer == null) {
            ...
            addView(progressContainer, MATCH_PARENT, MATCH_PARENT)
        }
    ...
    private suspend fun setImage() {
        ...
        try {
            ...
            if ((blockingReload || requireUpscaledImage) && pageUpscaler.isEnabled() && !cacheAvailableAfterLoad) {
                ...
                if (cacheAvailableAfterLoad) {
                    reloadImage(forceBlocking = false, seamless = true)
                } else if (!cacheAvailableBeforeLoad) {
    ...
    private fun awaitUpscaledImageAndReload() {
        cacheWatchJob?.cancel()
        cacheWatchJob = scope.launch {
            repeat(1_200) {
                if (!pageUpscaler.isEnabled()) {
                    return@launch
                }
