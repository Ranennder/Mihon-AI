...
class ReaderPreferences(
    ...
    val upscalePagesX2: Preference<Boolean> = preferenceStore.getBoolean("pref_reader_ai_x4", false)
    val aiBackendMode: Preference<AiBackendMode> = preferenceStore.getEnum("pref_reader_ai_backend", AiBackendMode.GPU)
    val remoteAiBaseUrl: Preference<String> = preferenceStore.getString("pref_reader_ai_remote_url", "")
    ...
    fun setSelectedAiBackendMode(mode: AiBackendMode) {
        aiBackendMode.set(normalizeAiBackendMode(mode))
    }

    fun migrateLegacyAiBackendMode() {
        val storedMode = aiBackendMode.get()
        val normalizedMode = normalizeAiBackendMode(storedMode)
        if (storedMode != normalizedMode) {
            aiBackendMode.set(normalizedMode)
        }
    }
    ...
    enum class AiBackendMode(
        ...
        CPU(MR.strings.reader_ai_backend_cpu),
        GPU(MR.strings.reader_ai_backend_gpu),
        NPU(MR.strings.reader_ai_backend_npu),
        REMOTE(MR.strings.reader_ai_backend_remote),
    }

    enum class RemoteAiModel(
        val titleRes: StringResource,
        val companionModelName: String,
        val cacheKey: String,
        val companionScale: Int,
    ) {
        FAST(
            titleRes = MR.strings.reader_ai_remote_model_fast,
            companionModelName = "realesr-animevideov3",
            cacheKey = "fast",
            companionScale = 2,
        ),
        DETAILED(
            titleRes = MR.strings.reader_ai_remote_model_detailed,
    ...
    companion object {
        ...
        val SelectableAiBackendModes = listOf(
            AiBackendMode.GPU,
            AiBackendMode.REMOTE,
        )

        fun normalizeAiBackendMode(mode: AiBackendMode): AiBackendMode {
            return when (mode) {
                AiBackendMode.CPU,
                AiBackendMode.NPU,
                -> AiBackendMode.GPU
